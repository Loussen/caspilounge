...
if($_POST) // Add && edit
{
    ...
    while($row=mysqli_fetch_assoc($sql))
    {
        ...
        $button_text="button_text_".$row["id"]; $button_text=mysqli_real_escape_string($db,htmlspecialchars($$button_text));

        $time = time();

        if(mysqli_num_rows(mysqli_query($db,"select id from $do where lang_id='$row[id]' $add_where"))>0 && $edit>0)
        {
            mysqli_query($db,"update $do set title='$title',button_text='$button_text',button_link='$button_link',order_number='$last_order',updated_at='$time' where lang_id='$row[id]' $add_where");
        }
        else
        {
            mysqli_query($db,"insert into $do values (0,'','$title','$button_text','$button_link','$last_order','$active', '$row[id]', '$auto_id', '$time',0) ");
        }
    }
    ...
    if($image_tmp!="")
    {
        ...
        if($image_access==true)
        {
            ...
            if($edit>0)
            {
                ...
                @unlink("../images/sliders/".$old_image_name['image_name']);
            }

            move_uploaded_file($image_tmp,'../images/sliders/'.$image_upload_name);

            compress('../images/sliders/'.$image_upload_name, '../images/sliders/'.$image_upload_name, 80);
...
if($delete>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete' "))>0)
{
    ...
    @unlink('../images/sliders/'.$data["image_name"]);
...
elseif($delete_img>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete_img' and image_name!='' "))>0)
{
    ...
    @unlink('../images/sliders/'.$info_image_name['image_name']);
}
    ...
    <div class="content">
            ...
            <?php
                ...
                while($row=mysqli_fetch_assoc($sql))
                {
                    ...
                    echo 'Title : <br />
                      ...
                      <br /><br />
                      Button text : <br />
                      <textarea name="button_text_'.$row["id"].'" cols="50" rows="5">'.$information["button_text"].'</textarea>                    
                      <br /><br />
                    </div>
                      ';
                }
                ...
                echo '<div class="'.$hide.'">';
                echo 'Button link : <br />
                    <input type="text" name="button_link" value="'.$information["button_link"].'" style="width:800px" /><br /><br />
                    </div>';
                ...
                {
                    $image='<ul class="media_photos" style="margin-left:450px;margin-top:-25px; margin-right: 40px;"><li><a rel="slide" href="../images/sliders/'.$information["image_name"].'?rand='.rand(0,10000).'" title="">
                    Current image : <img src="../images/sliders/'.$information ["image_name"].'?rand='.rand(0,10000).'" alt="" width="120" /></a>
                    <br class="clear" />
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            echo '<tr>
					...
					<td><ul class="media_photos" style="padding:0;margin-top:0px;margin-bottom:-65px"><li><a rel="slide" href="../images/sliders/'.$row["image_name"].'" title=""><img src="../images/sliders/'.$row["image_name"].'?rand='.rand(0,10000).'" width="100" height="60"></a></li></ul></td>
					<td>
