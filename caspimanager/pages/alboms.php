...
if($_POST) // Add && edit
{
    ...
    while($row=mysqli_fetch_assoc($sql))
    {
        ...
        {
            mysqli_query($db,"update $do set title='$title',updated_at='$time' where lang_id='$row[id]' $add_where");
        }
        else
        {
            mysqli_query($db,"insert into $do values (0,'$title','','$last_order','$active', '$row[id]', '$auto_id', '$time',0) ");
        }
    }
...
elseif($delete_img>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete_img' and image_name!='' "))>0)
{
    ...
    @unlink('../images/alboms/'.$info_image_name['image_name']);
}
    ...
    <div class="content">
        ...
        <form action="index.php?do=<?php echo $do; ?>&page=<?php echo $page; if($edit>0) echo '&edit='.$edit; ?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            ...
            if($information["image_name"]!="" && $edit>0)
            {
                $image='<ul class="media_photos" style="margin-left:450px;margin-top:-25px; margin-right: 40px;"><li><a rel="slide" href="../images/alboms/'.$information["image_name"].'?rand='.rand(0,10000).'" title="">
                    Current image : <img src="../images/alboms/'.$information ["image_name"].'?rand='.rand(0,10000).'" alt="" width="120" /></a>
                    <br class="clear" />
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            echo '<tr>
					...
					<td><ul class="media_photos" style="padding:0;margin-top:0px;margin-bottom:-50px"><li><a rel="slide" href="../images/alboms/'.$row["image_name"].'" title=""><img src="../images/alboms/'.$row["image_name"].'?rand='.rand(0,10000).'" width="100" height="60"></a></li></ul></td>
					<td>
