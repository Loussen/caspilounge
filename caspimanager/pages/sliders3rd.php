    ...
    if($image_tmp!="")
    {
        ...
        if($image_access==true)
        {
            ...
            if($edit>0)
            {
                ...
                @unlink("../images/sliders3rd/".$old_image_name['image_name']);
            }

            move_uploaded_file($image_tmp,'../images/sliders3rd/'.$image_upload_name);

            compress('../images/sliders3rd/'.$image_upload_name, '../images/sliders3rd/'.$image_upload_name, 80);
...
if($delete>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete' "))>0)
{
    ...
    @unlink('../images/sliders3rd/'.$data["image_name"]);
...
elseif($delete_img>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete_img' and image_name!='' "))>0)
{
    ...
    @unlink('../images/sliders3rd/'.$info_image_name['image_name']);
}
...
<div class="onecolumn">
    ...
    <div class="content">
        ...
        <form action="index.php?do=<?php echo $do; ?>&page=<?php echo $page; if($edit>0) echo '&edit='.$edit; ?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            ...
            {
                $image='<ul class="media_photos" style="margin-left:450px;margin-top:-25px; margin-right: 40px;"><li><a rel="slide" href="../images/sliders3rd/'.$information["image_name"].'?rand='.rand(0,10000).'" title="">
                    Current image : <img src="../images/sliders3rd/'.$information ["image_name"].'?rand='.rand(0,10000).'" alt="" width="120" /></a>
                    <br class="clear" />
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            echo '<tr>
					...
					<td><ul class="media_photos" style="padding:0;margin-top:0px;margin-bottom:-40px"><li><a rel="slide" href="../images/sliders3rd/'.$row["image_name"].'" title=""><img src="../images/sliders3rd/'.$row["image_name"].'?rand='.rand(0,10000).'" width="100" height="60"></a></li></ul></td>
