<div class="col-md-8 col-md-push-4 bwp-posts-col bwp-sidebar-left">
    <div class="bwp-posts-container" role="main">
        ...
        <div class="bwp-archive-heading">
            <h1 class="bwp-archive-title"><?=$lang23?>: <?=$current_category_name?></h1>
            <div class="bwp-archive-heading-line"></div>
        </div>
        <!-- end archive heading -->

        <?php
            foreach ($result_news_by_categories_arr as $item)
            {
                ?>
                <!-- post - aside format -->
                    ...
                    <div class="bwp-post-wrap bwp-post-aside-format">
                        ...
                        <header class="bwp-post-header">
                            ...
                            <div class="bwp-post-categories">
                                ...
                                <a href="<?=SITE_PATH."/category/".slugGenerator($current_category_name)."-".$current_category_id?>" rel="category tag"><?=$current_category_name?></a>
                            </div>
                            <!-- end categories -->
        ...
        if($count_rows > $limit)
        {
            ...
            <nav class="navigation pagination" role="navigation">
                ...
                <div class="nav-links">
                    ...
                    {
                        ?>
                        <a class="first page-numbers" href="<?=SITE_PATH?>/category/<?=slugGenerator($current_category_name) . '-' . $current_category_id.'/1'?>">
                            <i class="fa fa-step-backward"></i>
                        </a>
                        <a class="previous page-numbers" href="<?=SITE_PATH?>/category/<?=slugGenerator($current_category_name) . '-' . $current_category_id.'/'.($page-1)?>">
                    ...
                    for ($i = $page - $show; $i <= $page + $show; $i++)
                    {
                        if ($i > 0 && $i <= $max_page)
                        {
                            ...
                            {
                                ?>
                                <a class='page-numbers' href='<?=SITE_PATH?>/category/<?=slugGenerator($current_category_name) . '-' . $current_category_id.'/'.$i?>'><?=$i?></a>
                                <?php
                            }
                        }
                    }
                    ...
                    {
                        ?>
                        <a class="next page-numbers" href="<?=SITE_PATH?>/category/<?=slugGenerator($current_category_name) . '-' . $current_category_id.'/'.($page + 1)?>">
                            <i class="fa fa-caret-right"></i>
                        </a>
                        <a class="last page-numbers" href="<?=SITE_PATH?>/category/<?=slugGenerator($current_category_name) . '-' . $current_category_id.'/'.$max_page?>">
